...
const createChildeCategory=async(req,res)=>{
    try {
        ...
        res.status(201).json({
            message:'created',data:child,success:true,
        });
    } catch (error) {
...
const getChildCategoryId=async(req,res)=>{
    try {
        const child=await subchildcategoryService.getChildCategoryId(req.params.childId);
        if(!child){
...
module.exports={deleteChildCategory,getChildCategoryId,getChildCategory,createChildeCategory}